import { createContext, useContext } from 'react';

export const LANGS = ['ENG', 'FR', 'ESP'];

const translations = {
  ENG: {
    'common.input': 'Input',
    'common.result': 'Result',
    'common.reset': 'Reset',
    'common.run': 'Run',
    'common.loading': 'Loading',
    'common.image': 'Image',
    'common.pick_folder': 'or pick a folder',
    'common.left': 'left',
    'common.settings': 'Settings',
    'common.database': 'Faces',
    'common.empty_database': 'No identities stored yet',
    'common.picture': 'picture',
    'common.filename': 'filename',
    'common.date': 'date',
    'common.gender': 'gender',
    'common.match': 'match',
    'common.delete': 'delete',
    'common.view': 'view',
    'common.caption': 'Caption',
    'common.hf_token': 'HF token',
    'common.saved': 'Saved',
    'common.language': 'Language',
    'common.dark_mode': 'Dark mode',
    'common.advanced': 'Advanced settings',
    'mode.edit': 'FlowID',
    'mode.remove-background': 'Background remover',
    'mode.flux2klein': 'FLUX.2 [klein]',
    'mode.flux2klein-fast': 'FLUX.2 [klein] — fast',
    'mode.identity': 'Identity encoding',
    'identity.summary': '{processed} images processed, {faces_found} faces found, {skipped} skipped (no face)',
    'edit.source': 'Source description',
    'edit.target': 'Target description',
    'edit.training_steps': 'Training steps',
    'edit.inversion_steps': 'Inversion steps',
    'edit.negative_prompt': 'Negative prompt',
    'edit.preservation': 'Image preservation',
    'edit.step.training': 'Training',
    'edit.step.edit': 'Edit',
    'flux.prompt': 'Prompt',
    'flux.inference_steps': 'Inference steps',
    'flux.diffusion_coefficient': 'Diffusion coefficient',
    'flux.num_images': 'Images',
    'flux.seed': 'Seed',
    'flux.width': 'Width',
    'flux.height': 'Height',
    'progress.Loading': 'Loading',
    'progress.Generating': 'Generating',
    'progress.Training': 'Training',
    'progress.Inverting': 'Inverting',
    'progress.Editing': 'Editing',
    'progress.Embedding': 'Embedding',
  },
  FR: {
    'common.input': 'Entrée',
    'common.result': 'Résultat',
    'common.reset': 'Réinitialiser',
    'common.run': 'Lancer',
    'common.loading': 'Chargement',
    'common.image': 'Image',
    'common.pick_folder': 'ou choisir un dossier',
    'common.left': 'restant',
    'common.settings': 'Paramètres',
    'common.database': 'Visages',
    'common.empty_database': 'Aucune identité enregistrée',
    'common.picture': 'photo',
    'common.filename': 'fichier',
    'common.date': 'date',
    'common.gender': 'genre',
    'common.match': 'comparer',
    'common.delete': 'supprimer',
    'common.view': 'voir',
    'common.caption': 'Légende',
    'common.hf_token': 'Token HF',
    'common.saved': 'Enregistré',
    'common.language': 'Langue',
    'common.dark_mode': 'Mode sombre',
    'common.advanced': 'Paramètres avancés',
    'mode.edit': 'FlowID',
    'mode.remove-background': 'Background remover',
    'mode.flux2klein': 'FLUX.2 [klein]',
    'mode.flux2klein-fast': 'FLUX.2 [klein] — fast',
    'mode.identity': 'Identity encoding',
    'identity.summary': '{processed} images traitées, {faces_found} visages trouvés, {skipped} ignorées (aucun visage)',
    'edit.source': "Description de l'image",
    'edit.target': 'Description souhaitée',
    'edit.training_steps': "Étapes d'entraînement",
    'edit.inversion_steps': "Étapes d'inversion",
    'edit.negative_prompt': 'Prompt négatif',
    'edit.preservation': "Préservation de l'image",
    'edit.step.training': 'Entraînement',
    'edit.step.edit': 'Édition',
    'flux.prompt': 'Prompt',
    'flux.inference_steps': "Étapes d'inférence",
    'flux.diffusion_coefficient': 'Coefficient de diffusion',
    'flux.num_images': "Nombre d'images",
    'flux.seed': 'Graine',
    'flux.width': 'Largeur',
    'flux.height': 'Hauteur',
    'progress.Loading': 'Chargement',
    'progress.Generating': 'Génération',
    'progress.Training': 'Entraînement',
    'progress.Inverting': 'Inversion',
    'progress.Editing': 'Édition',
    'progress.Embedding': 'Encodage',
  },
  ESP: {
    'common.input': 'Entrada',
    'common.result': 'Resultado',
    'common.reset': 'Reiniciar',
    'common.run': 'Ejecutar',
    'common.loading': 'Cargando',
    'common.image': 'Imagen',
    'common.pick_folder': 'o elegir una carpeta',
    'common.left': 'restante',
    'common.settings': 'Ajustes',
    'common.database': 'Rostros',
    'common.empty_database': 'Sin identidades guardadas',
    'common.picture': 'foto',
    'common.filename': 'archivo',
    'common.date': 'fecha',
    'common.gender': 'género',
    'common.match': 'comparar',
    'common.delete': 'eliminar',
    'common.view': 'ver',
    'common.caption': 'Descripción',
    'common.hf_token': 'Token HF',
    'common.saved': 'Guardado',
    'common.language': 'Idioma',
    'common.dark_mode': 'Modo oscuro',
    'common.advanced': 'Ajustes avanzados',
    'mode.edit': 'FlowID',
    'mode.remove-background': 'Background remover',
    'mode.flux2klein': 'FLUX.2 [klein]',
    'mode.flux2klein-fast': 'FLUX.2 [klein] — fast',
    'mode.identity': 'Identity encoding',
    'identity.summary': '{processed} imágenes procesadas, {faces_found} rostros encontrados, {skipped} omitidas (sin rostro)',
    'edit.source': 'Descripción original',
    'edit.target': 'Descripción deseada',
    'edit.training_steps': 'Pasos de entrenamiento',
    'edit.inversion_steps': 'Pasos de inversión',
    'edit.negative_prompt': 'Prompt negativo',
    'edit.preservation': 'Preservación de imagen',
    'edit.step.training': 'Entrenamiento',
    'edit.step.edit': 'Edición',
    'flux.prompt': 'Prompt',
    'flux.inference_steps': 'Pasos de inferencia',
    'flux.diffusion_coefficient': 'Coeficiente de difusión',
    'flux.num_images': 'Número de imágenes',
    'flux.seed': 'Semilla',
    'flux.width': 'Ancho',
    'flux.height': 'Alto',
    'progress.Loading': 'Cargando',
    'progress.Generating': 'Generando',
    'progress.Training': 'Entrenando',
    'progress.Inverting': 'Invirtiendo',
    'progress.Editing': 'Editando',
    'progress.Embedding': 'Codificando',
  },
};

export function translate(key, lang, params) {
  let str = translations[lang]?.[key] ?? translations.ENG[key] ?? key;
  if (params) {
    str = str.replace(/\{(\w+)\}/g, (_, k) => params[k] ?? '');
  }
  return str;
}

export const LangContext = createContext({
  lang: 'ENG',
  t: (k, p) => translate(k, 'ENG', p),
  setLang: () => {},
});

export function useLang() {
  return useContext(LangContext);
}
